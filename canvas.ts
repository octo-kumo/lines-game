...
class Line {
    ...
    constructor(normal_angle_deg: number, d = 0) {
...
if (!canvas) throw new Error("canvas#canvas missing");
...
function createLineByDirectionAngle(phi_deg: number, d = 0) {
    return new Line((phi_deg + 90) % 180, d);
}

function createLineByPointAndAngle(
    normal_angle_deg: number,
    rx: number,
    ry: number,
) {
    const rad = (normal_angle_deg * Math.PI) / 180;
    return new Line(normal_angle_deg, -(Math.cos(rad) * rx + Math.sin(rad) * ry));
}

function createLineByTwoRelativePoints(
    rx1: number,
    ry1: number,
    rx2: number,
    ry2: number,
    ...
    const na = -dy / len,
        nb = dx / len;
    const d = -(na * rx1 + nb * ry1);
    return new Line((Math.atan2(nb, na) * 180) / Math.PI, d);
}

const lines: Line[] = [];

let hover: Line | null = null;
let cside = 0;
let selected: Line | null = null;
let hside = 0;
...
function clipToHalfPlane(points: Point[], a: number, b: number, c: number) {
    const out: Point[] = [];
...
function insetPolygon(points: Point[], dist: number) {
    if (points.length < 3) return [];
    if (polygonArea(points) < 0) points = points.slice().reverse();
    const res: Point[] = [];
    const n = points.length;
    for (let i = 0; i < n; i++) {
        ...
        let nx2 = -dy2 / len2,
            ny2 = dx2 / len2;
        const ox1 = prev.x + nx1 * dist,
            oy1 = prev.y + ny1 * dist;
        const ox2 = curr.x + nx2 * dist,
            oy2 = curr.y + ny2 * dist;
...
function pointOnLine(line: Line) {
    if (Math.abs(line.b) > 1e-6) {
        const px = 0,
            py = -(line.c + line.a * px) / line.b;
        return { x: px, y: py };
    } else {
        const py = 0,
            px = -(line.c + line.b * py) / line.a;
        return { x: px, y: py };
    }
}

function lineDirectionUnit(line: Line) {
    const dx = -line.b,
        dy = line.a;
    const len = Math.hypot(dx, dy);
...
function drawLineSegmentFromLine(line: Line, extendMult = 2) {
    const p = pointOnLine(line),
        dir = lineDirectionUnit(line);
...
canvas.addEventListener("mousemove", (e) => {
    ...
    if (selected && minDist > 10) {
        ...
        if (side !== 0) {
            const poly =
                side === 1
                    ? clipToHalfPlane(viewport, selected.a, selected.b, selected.c)
                    : clipToHalfPlane(viewport, -selected.a, -selected.b, -selected.c);
...
canvas.addEventListener("click", (e) => {
    ...
    if (clicked) {
        if (clickedSide === 0) selected = selected === clicked ? null : clicked;
...
function level2(spacingVal: number, count: number) {
    return [0, 60, 120].flatMap((n) =>
        Array.from(
            { length: count * 2 + 1 },
            (_, i) => new Line(n, (i - count) * spacingVal),
        ),
    );
}
